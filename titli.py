...
API_ID = os.environ.get("API_ID", "26469668")
API_HASH = os.environ.get("API_HASH", "cd3041c11c38886f61f583f175dab8e3")
STRING = os.environ.get("STRING", None)
OWNER_ID = os.environ.get("OWNER_ID", "6350398024")
...
async def chatbot(_, message: Message):
    if (len(message.command) > 1) and (message.command[1].lower() == "on"):
        ...
        if is_alt:           
            await message.reply_text(f"Titli Chatbot Is Already Enabled")
        else:
            chats_col.insert_one({"_id": message.chat.id})
            await message.reply_text(f"Titli ChatBot Is Enable!")
        ...
        if is_alt:
            ...
            await message.reply_text(f"Titli Chatbot Disabled!")
        else:
            await message.reply_text(f"Titli ChatBot Is Already Disabled")
...
print("Your Advanced Chatbot Is Now Ready! Join @TheSanjanaSupport")

bot.start()
bot.join_chat("@TheSanjanaSupport")
idle()
bot.stop()
